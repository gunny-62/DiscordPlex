...
json Discord::createActivity(const MediaInfo &info)
{
    ...
    {
        // Always prefer TMDB URLs if available
        if (info.artPath.find("image.tmdb.org") != std::string::npos)
        {
            assets["large_image"] = info.artPath;
            LOG_INFO("Discord", "Using TMDB artwork URL: " + info.artPath);
        }
        // Fall back to Plex URLs if they're not local-only
        else if (info.artPath.find("/photo/:/transcode") != std::string::npos)
        {
            if ((info.artPath.find("127.0.0.1") != std::string::npos) || 
                (info.artPath.find("localhost") != std::string::npos) ||
                (info.artPath.find("192.168.0.") != std::string::npos) ||
                (info.artPath.find("192.168.1.") != std::string::npos))
            {
                LOG_WARNING("Discord", "Local network Plex transcoder URL detected, using static Plex asset instead");
                assets["large_image"] = "plex";
            }
            else
            {
                assets["large_image"] = info.artPath;
                LOG_INFO("Discord", "Using Plex artwork URL: " + info.artPath);
            }
        }
        else
        {
